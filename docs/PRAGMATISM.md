...
10. `HYDRATION.md`: для межмодульных данных включить anti-storm минимум (`negative cache` + `request collapse`) в сценариях, где выбран hydration-подход.
11. `read-provider registry`: использовать для контрактного sync-read между модулями, когда нужна синхронная проверяемая зависимость без полной гидрации.
12. `TRANSACT.md`: включать только если модулю реально нужен транзакционный режим шины.
